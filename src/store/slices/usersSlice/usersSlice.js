...
const usersSlice = createSlice({
    ...
    reducers: {
        logIn(state, {payload: {login, password}}){
            state.currentUser = state.usersData.find(user => (user.email === login && user.password === password) ||
                                                                (user.username === login && user.password === password)) || null
        },
        logOut(state){
        ...
        delPost(state, {payload}){
            ...
            state.currentUser.posts = [...payload]
            state.usersData[userIdx].posts = [...payload]
        }
    },
    extraReducers: {
